...
<h4>✅ &nbsp;Why</h4> <code>Person.objects.filter(id__in=ids_to_delete).update(is_deleted=True)</code> <h4>is not same as</h4> <code>Person.objects.filter(id__in=ids_to_delete).delete()</code>
