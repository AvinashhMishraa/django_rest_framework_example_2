...
| `/api/person/all/` <br><br> `/api/person/?show_deleted=true` <br> `/api/person/?show_deleted=false`<br> | Separate endpoint for Admin only : returns alive + soft-deleted <br><br><br> Toggle via query param instead of separate endpoint<br>which makes it even more flexible  |
